...
public class Coordinates
{
    ...
    public double DistanceTo(Location destination)
    {
        ...
        var deltaLat = (destination.Coordinates.Latitude - Latitude) * (Math.PI / 180.0);
        var deltaLon = (destination.Coordinates.Longitude - Longitude) * (Math.PI / 180.0);
        var havFunc = Math.Sin(deltaLon / 2) * Math.Sin(deltaLon / 2) * Math.Cos(Latitude * (Math.PI / 180.0)) * Math.Cos(destination.Coordinates.Latitude * (Math.PI / 180.0))
